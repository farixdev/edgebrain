...
function ProjectCard({
  ...
  const isReference = "placeholder" in project && project.placeholder;
  const disclosure =
    "disclosure" in project && typeof project.disclosure === "string"
      ? project.disclosure
      : null;
    ...
    >
        ...
        >
          {"thumbnail" in project && project.thumbnail ? (
            ...
            <>
              ...
              {/* This flag exists to mark a project as NOT a named client
                  engagement, so the label it drives has to say that. It used
                  to read "Featured Build", which made the two invented
                  projects the only ones on the page carrying a credibility
                  badge — and at 20% opacity, unreadable either way. */}
              {isReference && (
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          {isReference && disclosure && (
            <p className="mt-2 text-sm text-[var(--color-mute)] max-w-sm">
              {disclosure}
            </p>
          )}
        </div>
